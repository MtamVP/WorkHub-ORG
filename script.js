...
function renderEventsForSelectedDate() {
    ...
    dailyEvents.forEach(event => {
        ...
        div.innerHTML =
            ...
            '<div class="event-title">' + escapeHtml(event.title) + (recurrenceLabel ? ' <i class="fa-solid fa-rotate text-muted" style="font-size:0.75em;" title="' + recurrenceLabel + '"></i>' : '') + (isGoogleSynced ? ' <i class="fa-brands fa-google text-muted" style="font-size:0.75em;" title="Đồng bộ từ Google Calendar"></i>' : '') + '</div>' +
...
const PERSONAL_SHIM = {
  openModal: (id) => showModal(id),
  closeModal: (id) => hideModal(id),
  accent: 'var(--gold)',
  me: () => ({ email: (typeof chatUser !== 'undefined' && chatUser) ? chatUser.email : null, groupKey: (typeof activeGroup !== 'undefined' ? activeGroup : null) }),
  goToMyTasks: () => { const n = document.querySelector('.nav-item[data-section="mytasks"]'); if (n) n.click(); }
};
