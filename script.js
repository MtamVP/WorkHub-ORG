...
// -------------------- Phase F: Backup & Restore (admin-only) --------------------
// Same cosmetic-gate-then-load pattern as loadAuditLog()/loadOrgUnits() above — real
// enforcement is each table's own RLS (a non-admin's backup legitimately captures
// partial/zero rows for admin-gated tables, since local-backup.js runs under the
// logged-in user's own session, not a service-role key).
    ...
    navItems.forEach(item => {
        item.addEventListener('click', function (e) {
            ...
            if (chatUser) {
                ...
                // D3b2. Tab Sao lưu & Phục hồi (admin)
                if (sectionName === 'backup-restore') {
                    if (typeof loadBackupRestore === 'function') loadBackupRestore();
                }
...
// Không phải personal_items — kéo trực tiếp từ dữ liệu nhóm (listMyTasks, đúng nguồn
// "Việc của tôi" đang dùng) để "việc đang giao cho bạn" có mặt ngay trong Không Gian
// Riêng mà không cần đổi section. Không ghi gì ngược lại khu chung.
async function renderRelatedPanel() {
    const listEl = document.getElementById('personal-hub-list');
    if (!listEl) return;
    listEl.innerHTML = skeletonListItems(3);

    const relatedEmail = (typeof chatUser !== 'undefined' && chatUser) ? chatUser.email : null;
    if (!relatedEmail) {
    ...
    try {
        const response = await callGAS('listMyTasks', { email: relatedEmail, groupKey: activeGroup });
        if (response.status !== 'success') throw new Error(response.message);
        const tasks = response.data || [];
        if (tasks.length === 0) {
      ...
      <div class="personal-item-card personal-note-card" onclick="goToTaskInProject('${escapeHtml(escapeJs(t.project_id))}')">
        ...
        <button type="button" class="btn btn-outline" onclick="document.querySelector('.nav-item[data-section=&quot;mytasks&quot;]').click()"><i class="fa-solid fa-arrow-right"></i> Xem tất cả trong Việc của tôi</button>
      </div>`;
    } catch (err) {
