...
const API = {
    ...
    user: {
        listAll: async () => {
            if (!sbClient) return [];
            const { data, error } = await sbClient.from('users').select('id, email, nickname, group_key, active, created_at').order('created_at', { ascending: false }).limit(1000);
            if (error) throw error;
            return data;
        },
    ...
    reporting: {
        summary: async (filters = {}) => {
            ...
            if (filters.orgUnitId) {
                const { data: descendantRows } = await sbClient.rpc('org_unit_descendant_ids', { p_root: filters.orgUnitId });
                const ids = (descendantRows || []).map(r => r.id);
                pq = ids.length ? pq.in('org_unit_id', ids) : pq.eq('id', '__none__');
            }
        ...
        projects: async (filters = {}) => {
            ...
            if (filters.orgUnitId) {
                const { data: descendantRows } = await sbClient.rpc('org_unit_descendant_ids', { p_root: filters.orgUnitId });
                const ids = (descendantRows || []).map(r => r.id);
                q = ids.length ? q.in('org_unit_id', ids) : q.eq('id', '__none__');
            }
...
const MUTATING_ACTIONS = new Set([
    ...
    'provisionUser', 'updateUserGroup', 'removeUser', 'setUserActive', 'updateNickname', 'updateMemberRole',
...
(function initPwaInstallPrompt() {
    ...
    function showButton() {
        ...
        btn.style.cssText = 'position:fixed;right:20px;bottom:20px;z-index:9999;display:flex;align-items:center;gap:8px;' +
            'background:#003366;color:#FFFFFF;border:none;border-radius:999px;padding:12px 18px;' +
