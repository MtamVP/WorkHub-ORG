...
const API = {
    ...
    user: {
        listAll: async () => {
            if (!sbClient) return [];
            const { data, error } = await sbClient.from('users').select('id, email, nickname, group_key, created_at').order('created_at', { ascending: false }).limit(1000);
            if (error) throw error;
            return data;
        },
...
const MUTATING_ACTIONS = new Set([
    ...
    'provisionUser', 'updateUserGroup', 'removeUser', 'updateNickname', 'updateMemberRole',
