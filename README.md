...
Ứng dụng quản lý task/dự án cho tổ chức/nhóm chung của hệ sinh thái **WorkHub**, kèm chatbot RAG tích hợp. Là 1 trong 3 app độc lập cùng hệ sinh thái (xem [WorkHub-Fin](https://github.com/MtamVP/WorkHub-Fin), [WorkHub-Sci](https://github.com/MtamVP/WorkHub-Sci)).
...
> ⚠️ **Lưu ý cấu trúc riêng của repo này**: UI thật của app nằm ở [`dashboard/index.html`](./dashboard/index.html), **không phải** `index.html` ở gốc repo (đó chỉ là trang đăng nhập). Trước khi sửa gì liên quan UI/script tag, đọc kỹ phần này trong `PROJECT_STATUS.md`.

## Clone repo

```bash
git clone https://github.com/MtamVP/WorkHub-ORG.git
cd WorkHub-ORG
```
...
- **Frontend**: vanilla JavaScript SPA, không build step, không framework — `index.html` (trang đăng nhập) + `dashboard/index.html` (app thật) dùng chung `script.js` + `api.js` ở gốc repo + `style.css`.
- **Backend**: [Supabase](https://supabase.com) (Postgres + Auth + Storage), dùng chung project với WorkHub-Fin và WorkHub-Sci. Không có server riêng — mọi logic nghiệp vụ nằm trong JS client hoặc DB trigger.
...
Không có bước `npm run dev` riêng — app không dùng bundler nên mọi thay đổi ở `script.js`/`api.js`/`dashboard/index.html`/`style.css` chỉ cần build lại (hoặc mở thẳng qua 1 static file server bất kỳ để test nhanh phần giao diện, không cần Tauri).
