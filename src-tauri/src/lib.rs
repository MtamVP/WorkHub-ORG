...
const SYNC_IGNORE_NAMES: [&str; 5] = [".git", "node_modules", "Thumbs.db", ".DS_Store", "desktop.ini"];
...
fn is_sync_ignored(path: &Path) -> bool {
  path.components().any(|c| {
    ...
    SYNC_IGNORE_NAMES.iter().any(|ignored| name.as_ref() == *ignored)
  })
}
...
#[tauri::command]
fn sync_write_file(root: String, relative_path: String, content_base64: String) -> Result<(), String> {
  let full = PathBuf::from(&root).join(&relative_path);
...
#[tauri::command]
fn sync_start_watch(root: String, app: AppHandle, state: State<SyncWatcherState>) -> Result<(), String> {
  ...
  tauri::Builder::default()
    ...
    .plugin(
      tauri_plugin_sql::Builder::default()
        .add_migrations("sqlite:workhub-org-cache.db", migrations)
        .build(),
    )
    .plugin(
