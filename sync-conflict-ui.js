(function () {
    ...
    function updateBadge(count) {
        var anchor = document.querySelector('.noti-btn-container');
        if (!anchor) return;
        ...
        if (!badge) {
            ...
            badge.style.cssText = 'position:absolute; top:-4px; right:-4px; background:#e53e3e; color:#fff; ' +
                'font-size:10px; line-height:1; border-radius:999px; padding:2px 5px; font-weight:600; cursor:pointer; z-index:5;';
            // Click the badge itself (not the whole bell) to open the conflict modal, so it
            // doesn't fight with the bell's own Bootstrap offcanvas-toggle click handler.
    ...
    function openConflictModal() {
        renderConflictList();
        if (typeof window.showModal === 'function') window.showModal('sync-conflict-modal');
    }
